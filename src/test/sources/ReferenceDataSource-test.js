...
describe('ReferenceDataSource', function() {
  ...
  beforeEach(() => {
    // define ReferenceDataSource
    source = ReferenceDataSource.create({
      url: '/reference',
      contigList: [{
        name:"chrM",
        length: 1000
  ...
  it('should fetch contigs', function() {
    ...
    contigs = _.map(contigs, contig => contig.name);
  ...
  it('should fetch base pairs', function(done) {
    ...
    source.on('newdata', () => {
      ...
      str = source.getRangeAsString(range);
      expect(str).to.equal('NGTT');
